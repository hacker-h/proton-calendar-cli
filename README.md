...
Conferencing, attachments, and categories are explicit unsupported boundaries until their private API shapes can be observed safely. Proton's Meet docs describe encrypted Meet links generated by the web app, with optional auto-add only when an event has invitees: <https://proton.me/support/calendar-meet>. Proton's Zoom docs require a paid Proton plan, a Zoom account, and sometimes organization admin approval, so Zoom creation is not free-account-testable: <https://proton.me/support/calendar-zoom>. Appointment scheduling can use Proton Meet but is paid-plan gated, and non-Meet links are plain location text rather than auto-created Zoom metadata: <https://proton.me/support/calendar-appointment-scheduling>. The CLI therefore treats `URL`, `ATTACH`, `CATEGORIES`, `CONFERENCE`, organizer/attendee data, and arbitrary ICS passthrough as unsupported import fields instead of silently dropping or mutating them.

Special calendars are list/read-only unless Proton returns enough normal event data for an existing calendar. `pc calendars -o json` and `GET /v1/calendars` preserve observed metadata such as `type`, `flags`, `readOnly`, and `syncStatus` from Proton's calendar list so automations can identify subscribed or managed calendars without a quota-consuming mutation. Proton documents subscribed URL calendars as view-only, synced every 4-16 hours, and counting toward calendar limits: <https://proton.me/support/subscribe-to-external-calendar>. Public holiday calendars are web/Android-managed, count toward plan quotas, and are not normal event fields: <https://proton.me/support/public-holiday-calendars>. Automatic birthday calendars remain undocumented/unsupported; use a normal imported/custom calendar for birthday events until Proton exposes a stable Calendar surface. This project does not create subscribed calendars, holiday calendars, birthday calendars, or share links, and live checks for those surfaces stay behind `PROTON_LIVE_ENABLE_SUBSCRIBED_CALENDARS`, `PROTON_LIVE_ENABLE_HOLIDAY_CALENDARS`, and `PROTON_LIVE_ENABLE_BIRTHDAY_CALENDARS`.
